...
function app_timeline (opts = default_opts, protocol) {
  ...
  async function onscroll () {
    ...
    if(cardfilter.length > card_index && timeline_height < timeline_scrollTop + 1000){
  ...
  async function set_scroll (data) {
    ...
    if (data.filter === 'YEAR'){
      //loading the cards
      const old_card_index = card_index
      fragment = document.createDocumentFragment()
      ...
      //If more than 100 cards are loading then show a loading screen
      if(card_index > old_card_index + 100){
        console.time('hello')
        timeline_wrapper.classList.add('hide')
        empty_wrapper.classList.add('show')
        timeline_wrapper.appendChild(fragment)
        //scroll to the cards
        status[data.filter] = data.value
        
        setTimeout(() => {
          empty_wrapper.classList.remove('show')
      ...
      }//else don't show loading screen
      else{
        timeline_wrapper.appendChild(fragment)
        //scroll to the cards
        status[data.filter] = data.value
