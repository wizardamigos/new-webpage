...
function app_timeline (opts = default_opts, protocol) {
  ...
  { // timeline cards
    const on = {}
    timeline_cards = cards_data.slice(0, 48).map(make_card)
  ...
  function make_card (card_data) {
    const protocol = use_protocol(`card_${card_index}`)({ state, on })
    ...
    if (year_cache !== slice) {
      ...
      while(sorting ? latest <= oldest : latest >= oldest){
        ...
        card_groups.push(separator)
        timeline_wrapper.append(separator)
    ...
    element.idx = card_index
    card_index++
    element.dataset.title = card_data.title
    return element
  }
  ...
  async function onscroll (event) {
    ...
    if(cardfilter.length > counter && timeline_height < timeline_scrollTop + 1000){
      timeline_cards.push(...cardfilter.slice(counter, counter+=10).map(make_card))
    }
  ...
  async function set_scroll (data) {
    ...
    if (data.filter === 'YEAR'){
      //loading the cards
      for (const card_data of cardfilter.slice(counter)){
        if(Number(card_data.date.slice(-5)) < data.value)
          break
  ...
  async function set_filter (data) {
    ...
    timeline_cards = cardfilter.slice(0, 48).map(make_card)
    counter = 48
