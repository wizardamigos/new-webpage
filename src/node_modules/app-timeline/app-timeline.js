...
function app_timeline (opts = default_opts, protocol) {
  ...
  async function prepend_cards(start, end){
    scroll_dir = false
    fragment = document.createDocumentFragment()
    timeline_cards.push(...cardfilter.slice(start, end).reverse().map(make_card))
  ...
  async function set_scroll (data) {
    ...
    if (data.filter === 'YEAR'){
      ...
      timeline_cards = []
      status.separators = []
      card_index = card_index < cardfilter.length && Number(cardfilter[card_index].date.slice(-5)) > data.value ? card_index : 0
      ...
      prepend_cards(card_index - scroll_index, card_index)
      append_cards(card_index, card_index + 10)
      //scroll to the cards
